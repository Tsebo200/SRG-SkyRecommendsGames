...
export default function RecommendationsScreen() {
  ...
  const checkForSteamProfile = async () => {
    try {
      const saved = await AsyncStorage.getItem('user_steam_account');
      if (!saved) return;

      // If a full URL was saved, try to extract a 17-digit SteamID64
      let steamId = saved;
      if (/^https?:\/\//i.test(saved)) {
        const match = saved.match(/(\d{17})/);
        if (match && match[1]) {
          steamId = match[1];
        }
      }

      // Basic validation for SteamID64
      if (/^\d{17}$/.test(steamId)) {
        setSteamProfile(steamId);
        // Auto-generate once to avoid race/dup calls
        if (!steamInitialized && !steamLoading) {
          setSteamInitialized(true);
          generateSteamRecommendations(steamId);
        }
      }
    } catch (e) {
      // Non-fatal
    ...
    try {
      ...
      if (Array.isArray(aiRecommendations.recommendations)) {
        ...
        try {
          ...
          let cleanResponse = String(aiRecommendations.recommendations).trim();
          ...
          const fallbackRecommendations: RecommendationGame[] = [{
            id: 0,
            slug: "ai-recommendations",
            name: "AI-Generated Recommendations",
            description: String(aiRecommendations.recommendations),
      ...
      if (error.message?.includes('timeout') || error.code === 'ECONNABORTED') {
        ...
        setError(`Failed to generate AI recommendations: ${error.message || 'Unknown error'}\n\nPlease check your backend server is running and accessible.`);
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: themeColors.background }]} edges={['top', 'bottom', 'left', 'right']}>
      <View style={[styles.header, { transform: [{ translateY: -20 }] }]}>
...
const styles = StyleSheet.create({
  ...
  listContainer: {
    padding: 16,
    paddingTop: 0,
    paddingBottom: 100, // ensure last item is fully visible above tab bar
  },
  row: {
    justifyContent: 'space-between',
  },
  ...
  emptyState: {
    ...
    paddingTop: 55, // Gap between header horizontal line and content
    paddingBottom: 100, // Add space for tab bar
  },
  emptyIconContainer: {
    marginBottom: 0,
