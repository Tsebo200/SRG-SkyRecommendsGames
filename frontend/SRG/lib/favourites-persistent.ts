...
export class PersistentFavouritesService {
  // Get user's ID for storage keys
  ...
  static async getFavourites(): Promise<FavouriteGame[]> {
    ...
    try {
      ...
      console.log('✅ Loaded favourites (deduplicated):', favourites.length, 'items');
      return favourites;
    } catch (error) {
  ...
  // Clear all favourites (local and Supabase)
  static async clearAllFavourites(): Promise<void> {
    try {
      console.log('🗑️ Clearing all favourites...');
      ...
      if (userId) {
        try {
          ...
          if (error) {
            console.log('⚠️ Failed to clear Supabase favourites:', error.message);
          } else {
            console.log('✅ Cleared Supabase favourites');
          }
        } catch (error) {
          console.log('⚠️ Failed to clear Supabase favourites:', error);
        }
      }
    } catch (error) {
